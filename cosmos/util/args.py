...
def add_workflow_args(p, require_name=True):
    ...
    p.add_argument('--default_max_attempts', '--default-max-attempts', '-a', type=int,
                   help="Default number of times to try running a Task that must succeed before the workflow fails", default=1)
