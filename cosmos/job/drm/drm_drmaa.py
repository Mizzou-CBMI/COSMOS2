    ...
    def submit_job(self, task):
        with get_drmaa_session().createJobTemplate() as jt:
            ...
            try:
                ...
                task.log.error(
                    "%s failed submission to %s with nativeSpecification=`%s`"
    ...
    def filter_is_done(self, tasks):
        ...
        while len(jobid_to_task):
            ...
            try:
                ...
                print(
                    "drmaa raised a naked Exception while "
...
def parse_drmaa_jobinfo(drmaa_jobinfo):
    ...
    cosmos_jobinfo = dict(
        ...
        percent_cpu=div(float(d.get("cpu", 0)), float(d.get("ru_wallclock", 0))),
    ...
    #
    if (
        int(drmaa_jobinfo["exitStatus"]) != 0
        or drmaa_jobinfo["hasSignal"]
        or drmaa_jobinfo["wasAborted"]
        or not drmaa_jobinfo["hasExited"]
    ):
