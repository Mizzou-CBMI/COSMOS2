...
class Recipe(object):
    ...
    def add_source(self, tasks, name=None):
        ...
        assert len(tags) == len(
            set(tags)), 'Duplicate inputs tags detected for {0}.  Tags within a recipe_stage must be unique.'.format(INPUT)
    ...
    def save_image(self, save_to=None):
        ...
        dag.add_nodes_from([n.label for n in self.recipe_stage_G.nodes()])
        for u, v, attr in self.recipe_stage_G.edges(data=True):
