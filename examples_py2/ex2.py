...
def recipe(workflow):
    ...
    echo_tasks = [
        workflow.add_task(
            func=echo, params=dict(word=word, out_txt="%s.txt" % word), uid=word
        )
    ...
    for echo_task in echo_tasks:
        ...
        for n in [1, 2]:
            cat_task = workflow.add_task(
                func=cat,
                params=dict(
                    in_txts=[echo_task.params["out_txt"]],
            ...
            word_count_task = workflow.add_task(
                ...
                params=dict(
                    in_txts=[Dependency(cat_task, "out_txt")],
    ...
    summarize_task = workflow.add_task(
        func=cat,
        params=dict(
            in_txts=[Dependency(t, "out_txt") for t in word_count_tasks],
            out_txt="summary.txt",
        ),
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-drm", default="local", help="", choices=("local", "drmaa:ge", "ge", "slurm")
    )
    p.add_argument(
        "-j", "--job-class", help="Submit to this job class if the DRM supports it"
    )
    ...
    if pygraphviz_available:
        ...
        draw_stage_graph(
            workflow.stage_graph(), "/tmp/ex1_task_graph.png", format="png"
        )
