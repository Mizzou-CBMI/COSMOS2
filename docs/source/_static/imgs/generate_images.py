...
stageB_tasks = workflow.add(ToolB(params=task.params, parents=[task]) for task in stageA_tasks)
...
stageB_tasks = workflow.add(
    ToolB(params=dict(j=j, **task.params), parents=[task]) for task in stageA_tasks for j in ["a", "b"]
)
draw_task_graph(workflow.task_graph(), "one2many.png", format="png")

workflow = cosmos.start("Many2One", "/tmp", check_output_dir=False)
stageA_tasks = workflow.add(ToolA(params=dict(i=i, j=j)) for i in [1, 2] for j in ["a", "b"])
get_i = lambda task: task.params["i"]
stageB_tasks = workflow.add(
    ToolB(params=dict(i=i), parents=list(tasks))
