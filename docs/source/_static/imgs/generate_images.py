...
stageA_tasks = workflow.add(ToolA(tags=dict(i=i))
                             for i in [1, 2])
stageB_tasks = workflow.add(ToolB(tags=task.tags, parents=[task])
...
stageA_tasks = workflow.add(ToolA(tags=dict(i=i))
                             for i in [1, 2])
stageB_tasks = workflow.add(ToolB(tags=dict(j=j, **task.tags), parents=[task])
                             for task in stageA_tasks
                             for j in ['a','b'])
draw_task_graph(workflow.task_graph(), 'one2many.png', format='png')

workflow = cosmos.start('Many2One', '/tmp', check_output_dir=False)
stageA_tasks = workflow.add(ToolA(tags=dict(i=i, j=j))
                             for i in [1, 2]
                             for j in ['a','b'])
get_i = lambda task: task.tags['i']
stageB_tasks = workflow.add(ToolB(tags=dict(i=i), parents=list(tasks))
