...
def echo(self, word, out_txt=out_dir('echo.txt')):
    return '{s[echo_path]} {word} > {out_txt}'.format(s=s, **locals())


def cat(self, inputs=find('txt$', n='>=1'), out_txt=out_dir('cat.txt')):
...
def paste(self, input_txts=find('txt$', n='>=1'), out_txt=out_dir('paste.txt')):
...
def word_count(self, chars=False, input_txts=find('txt$', n='>=1'), out_txt=out_dir('wc.txt')):
    c = ' -c' if chars else ''
...
def md5sum(self, in_file=find('.*', n=1), out_md5=out_dir('checksum.md5')):
